...
void wydbg_printName(PyObject *obj)
{
	...
	for (i = 0; (Py_TYPE(obj) -> tp_name[i]) != '\0'; i++){
		printf("%c", Py_TYPE(obj) -> tp_name[i]);
	}
	printf("\n");
}
...
//WYDBG IMFORMATION
		if(WYDBG_FLAG_TO == 1){
			printf("best_base: address of candidate = 0x%x\n", candidate);
			printf("best_base: name    of candidate = ");
			wydbg_printName(candidate);
		}
//WYDBG IMFORMATION
        if (winner == NULL) {
            winner = candidate;
...
//WYDBG IMFORMATION
	if(WYDBG_FLAG_TO == 1){
		printf("best_base: name of the final bestest obj = ");
...
//WYDBG IMFORMATION
    ...
    if (type->tp_itemsize || base->tp_itemsize) {
		...
		if(WYDBG_FLAG_TO == 1){
			printf("	extra_ivars return value01 = 0x%x\n", (t_size != b_size || type->tp_itemsize != base->tp_itemsize));
			printf("------- extra_ivars -------");
		}
	...
	if(WYDBG_FLAG_TO == 1){
		printf("	extra_ivars return value02 = 0x%x\n", (t_size != b_size));
		printf("------- extra_ivars -------");
	}
    return t_size != b_size;
}
...
//WYDBG IMFORMATION	
        ...
        for (i = 0; i < nbases; i++) {
            ...
            tmpTmp = _PyObject_GetAttrId(tmp, &PyId___mro_entries__);
...
//WYDBG IMFORMATION				
				...
				printf("\n");
				printf("	PyId___mro_entries__.object.tp_flags = 0x%x\n", Py_TYPE(dbgTmp01) -> tp_flags);
				printf("----------PyId___mro_entries__ _PyObject_GetAttrId---------\n");
				...
				if(tmp != NULL){
					...
					printf("\n");
					printf("	tmp.tp_flags = 0x%x\n", Py_TYPE(tmp) -> tp_flags);
				}
...
//WYDBG IMFORMATION	
 		...
 		if(WYDBG_FLAG_TO == 1) printf("Can run to here!\n");	//WYDBG IMFORMATION	
