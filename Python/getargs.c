	...
	va_list *p_va, int flags)
{
	...
	if (WYDBG_FLAG == 1) {
		...
		printf("    after while loop 1, level = %d, max =%d, min = %d, nargs = %d, compat = 0x%x\n", level, max, min, nargs, compat);
	}
	...
	for (i = 0; i < nargs; i++) {
		...
		if (WYDBG_FLAG == 1) {
			...
			printf("    address of stack[%d] = 0x%x, format = %c\n", i, (int)(stack[i]), *format);
		}
